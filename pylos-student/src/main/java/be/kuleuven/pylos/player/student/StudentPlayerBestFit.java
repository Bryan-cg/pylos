...
public class StudentPlayerBestFit extends PylosPlayer {
    ...
    public void doRemove(PylosGameIF game, PylosBoard board) {
        ...
        for (PylosSphere sphereToRemove : removableSpheres) {
            ...
            final double result = miniMaxRec();
            if (result > bestScore) {
    ...
    private double doMoveSim() {
        final boolean maximizingPlayer = PLAYER_COLOR == simulator.getColor();
        double bestScore = (maximizingPlayer) ? -9999 : 9999;
        final PylosPlayer player = (maximizingPlayer) ? this : OTHER;
        final PylosGameState prevState = simulator.getState();
        final PylosPlayerColor prevColor = player.PLAYER_COLOR;
        final List<PylosSphere> movableSpheres = getRemovableSpheres(player);
        ...
        for (PylosSphere sphere : movableSpheres) {
            for (PylosLocation location : getPossibleLocations(sphere)) {
                ...
                final double nextScore = miniMaxRec();
    ...
    public double doRemoveSim() {
        final PylosPlayerColor simulatorColor = simulator.getColor();
        final PylosPlayer simulatorPlayer = (simulatorColor == PLAYER_COLOR) ? this : OTHER;
    ...
    public void miniMax() {
        ...
        final List<PylosSphere> movableSpheres = getRemovableSpheres(this);
        movableSpheres.add(currentBoard.getReserve(this));
        for (PylosSphere sphere : movableSpheres) {
            PylosLocation prevLocation = sphere.getLocation();
            for (PylosLocation location : getPossibleLocations(sphere)) {
                simulator.moveSphere(sphere, location);
                final double score = miniMaxRec();
                if (score > bestScore) {
                    bestScore = score;
                ...
                if (prevLocation == null)
                    simulator.undoAddSphere(sphere, prevState, PLAYER_COLOR);
    ...
    private boolean makeSquare(PylosPlayer player, List<PylosLocation> allPossibleLocations) {
        ...
        if (!fullSquareLocations.isEmpty()) {
            ...
            for (PylosLocation fullSquareLocation : fullSquareLocations) {
                final List<PylosSphere> movableSpheresToLocation =
    ...
    /**
     * @return Evaluation function that emphasizes preserving the spheres of our player, making full squares of our spheres and
     * blocking spheres of opponent
     */
    private double evaluateBoard() {
        final double reserveScore = (-30 * (NUMBER_OF_SPHERES - currentBoard.getReservesSize(this))) +
    ...
    /**
     * @param player
     * @return nr of spheres on board which can not be moved (they're blocking squares or have
